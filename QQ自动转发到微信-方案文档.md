...
```
QQ群消息 ──NapCat(小号)──> HTTP上报 ──> Python中转服务 ──> 企业微信消息推送Webhook ──> 同步到个人微信
```

### 1.2 为什么是这个方案（关键决策依据）

| 维度 | 企业微信消息推送方案(推荐) | PC微信Hook方案(备选) |
|------|--------------------------|----------------------|
...
> "未认证"不影响消息推送 Webhook 功能使用，只是成员上限和部分高级功能受限，对本方案无影响。

### 1.4 企业微信消息如何同步到个人微信

注册企业微信后，在企业微信 App 中：
- 「我」→「设置」→「消息通知」→ 开启「企业微信消息同步到微信」
- 这样消息推送发的内容，你**个人微信会收到通知**，点开可直接查看
...
4. **Python中转服务**(FastAPI) 接收上报，判断：
   ...
   - 是否需要关键词过滤？
5. 格式化消息文本（加群名、发言人前缀）
6. 调用 **企业微信消息推送 Webhook**，POST JSON 推送
7. 企业微信群收到消息 → **同步到个人微信**
...
| 企业微信消息推送 | 接收Webhook请求，把消息发到群里 | 腾讯企业微信服务 |
...
### 3.3 企业微信消息推送 Webhook

- **是什么**：企业微信群里的消息推送功能（原"群机器人"），提供一个 URL，POST JSON 即可发消息
- **限制**：每个消息推送每分钟最多20条消息；单条消息文本最长4096字节
- **费用**：完全免费
...
4. **测试Webhook**（用curl验证可用）：
   ...
   curl 'https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=你的KEY' \
      ...
      -d '{"msgtype":"text","text":{"content":"测试消息：消息推送配置成功！"}}'
   ```
...
# ============ 企业微信 Webhook ============
# 替换为你自己的消息推送Webhook URL（5.1节第3步获取）
...
### Q2：企业微信群只有我一个人，能加消息推送吗？
A：能。企业微信一个人也能建群，消息推送功能与人数无关。

### Q3：能转发图片和文件吗？
A：当前代码仅转文本（图片会转成URL占位符）。完整图片转发需要：NapCat下载图片 → 上传到企业微信临时素材接口 → 用image类型推送。属于进阶功能，需要再开发。

### Q4：能转发到多个企业微信群吗？
A：能。配置多个Webhook URL，代码中按群号路由到不同消息推送即可。
